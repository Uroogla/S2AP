...
class Logic(ABC):
    ...
    @abstractmethod
    def can_access_aquaria_after_first_room(self, state):
        pass

    @abstractmethod
    def can_access_aquaria_second_half(self, state):
        pass
...
class BaseLogic(Logic):
    ...
    def can_enter_ocean(self, state):
        return self.can_access_summer_second_half(state) and self.can_satisfy_level_lock("Ocean Speedway", state) and state.has("Orb", self.world.player, 3)

    def can_enter_crush(self, state):
        has_sufficient_sparx = not self.world.options.enable_progressive_sparx_logic.value or self.has_sparx_health(1, state)
        is_open_world = self.world.options.enable_open_world.value
        return self.can_access_summer_second_half(state) and has_sufficient_sparx and (is_open_world or state.has("Summer Forest Talisman", self.world.player, 6))
    ...
    def can_access_sf_ladder(self, state):
        return self.can_access_summer_second_half(state) and self.can_climb(state)
    ...
    def can_access_aquaria_second_half(self, state):
        return self.can_swim(state) and (self.can_bypass_moneybags(state) or state.has("Moneybags Unlock - Aquaria Towers Submarine", self.world.player))
    ...
    def can_access_crystal_bridge(self, state):
        return self.can_bypass_moneybags(state) or state.has("Moneybags Unlock - Crystal Glacier Bridge", self.world.player)
...
class EasyLogic(Logic):
    ...
    def can_enter_crush(self, state):
        has_sufficient_sparx = not self.world.options.enable_progressive_sparx_logic.value or self.has_sparx_health(1, state)
        is_open_world = self.world.options.enable_open_world.value
        return self.can_access_summer_second_half(state) and has_sufficient_sparx and \
            (
                is_open_world or
    ...
    def can_access_summer_second_half(self, state):
        return self.can_swim(state) or self.can_double_jump(state)

    def can_access_sf_secret_ledge(self, state):
        return self.can_swim(state)

    def can_access_sf_ladder(self, state):
        return self.can_access_summer_second_half(state) and (self.can_climb(state) or self.can_swim(state))
...
class MediumLogic(Logic):
    ...
    def can_enter_ocean(self, state):
        return self.can_access_summer_second_half(state) and self.can_satisfy_level_lock("Ocean Speedway", state) and \
            (state.has("Orb", self.world.player, 3) or self.can_swim(state))
    ...
    def can_do_glimmer_indoor_lamps(self, state):
        # TODO: Add powerup locks logic
        return True

    def can_access_aquaria_after_first_room(self, state):
        # TODO: handle fireball backwards access
        return self.can_swim(state)
    ...
    def can_access_crystal_bridge(self, state):
        return self.can_double_jump(state) or self.can_bypass_moneybags(state) or state.has("Moneybags Unlock - Crystal Glacier Bridge", self.world.player)
